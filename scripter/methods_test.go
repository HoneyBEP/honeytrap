...
//TestGetAbTester tests the ab tester retrieval from a connection wrapper
func TestGetAbTester(t *testing.T) {
	abt, err := abtester.Dummy("test", toml.Primitive{})
	if err != nil {
		t.Fatal(err)
	}

	dummy, err := Dummy("test", WithAbTester(abt))
	if err != nil {
		t.Fatal(err)
	}

	ab, ok := dummy.(ScrAbTester)
	if !ok {
		t.Errorf("unable to retrieve scripter with ab tester")
	}
	got := getAbTest(ab, connectionWrapper.Conn, "test")()
